...
jal	insertion_sort		# selection_sort(a,n)
...
loop:
 ...
 addi $v1, $v1, 1
  ...
  jr      $ra            
		...
		#insertion sort
insertion_sort:
		addi	$sp, $sp, -20		# luu vao stack
		sw	$ra, 0($sp)		#luu dia chi tra ve cua thu tuc
		sw	$s0, 4($sp)		
		sw	$s1, 8($sp)
		sw	$s2, 12($sp)
		sw	$s3, 16($sp)

		move 	$s0, $a0		# dia chi cua mang
		li	$s1, 1			# i=1, $s1 la i 

		subi	$s2, $a1, 1		# lenght -1
isort_for:	bgt 	$s1, $s2, isort_exit	# if i > length-1 -> ketthuc
		...
		jal	while
		move	$s3, $v0		# return gia tri pos
		###
		move	$s4, $v1		# gia tri cua x
		###
		move	$a0, $s0		# array
		move	$a1, $s1		# i
		move	$a2, $s3		# pos
...
while:		move	$t0, $a0		# dia chi cua mang
		move	$t1, $a1		# mini = i
		move	$t2, $a2		# length
		...
		addi	$t5, $t1, -1		# pos = i-1
mini_for:	
		blt	$t5, $0, mini_if_exit	#t5 = pos < 0 -> break
		sll	$t6, $t5, 2		# i * 4
		add	$t6, $t6, $t0		# index = base array + i * 4		
		lw	$t7, ($t6)		# a[pos] = a[index]

		ble	$t7, $t4, mini_if_exit	# skip the if when a[pos] = a[i] <= x thi nhay ket thuc


		
		addi	$t6, $t6, 4		# dia chi pos+1
		sw	$t7, 0($t6)		# a[pos+1] = a[pos]
...
mini_if_exit:
		...
		move 	$v0, $t5		# return pos
		move    $v1, $t4		# ===> x de tranh bi mat
		jr	$ra
...
swap:		
		sll	$t2, $a2, 2		# pos * 4
		add	$t2, $t2, $a0		#  dia chi mang + pos * 4
		addi	$t2, $t2, 4		# pos +1
		
		add 	$t3, $a3, $0		#load x vo t3
