...
isort_for:	bgt 	$s1, $s2, isort_exit	# if i > length-1 -> exit loop
		...
		jal	while
		move	$s3, $v0		# return value of pos
		###
...
mini_for:	#bgt	$t5, $t2, mini_end	# go to min_end
		...
		lw	$t7, 0($t6)		# a[pos] = v[index]

		ble	$t7, $t4, mini_if_exit	# skip the if when a[pos] = v[i] <= x thi nhay ket thuc
...
swap:		sll	$t1, $a1, 2		# i * 4
		...
		#lw	$t0, 0($t1)		# v[i] load v[i] nhung no se thay doi
